...
libnjkafka_BatchResults libnjkafka_consumer_poll_each_message(libnjkafka_Consumer* consumer, int timeout_ms, libnjkafka_ConsumerRecord_Processor* message_processor) {
    ...
    for(int i = 0; i < records->count; i++) {
        ...
        int result = ((int (*)(libnjkafka_ConsumerRecord))message_processor)(record);
        if (result != 0) {
            printf("Failed to process message, bailing!\n");
            break;
        }
